...
func (p *Active) fieldPtr(args ...dialect.Field) []func(*Active) any {
	if len(args) == 0 {
		args = tblactive.ReadableFields
	}
	fs := make([]func(*Active) any, 0, len(args))
	for _, col := range args {
		if f, ok := activeFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Active) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := activeFieldToPtrFunc[col]; ok {
...
func (p *Active) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Active, bool, error) {
	defer rows.Close()
...
func (p *Active) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := activeFieldToValueFunc[arg]; has {
			value, isZero := f(p)
