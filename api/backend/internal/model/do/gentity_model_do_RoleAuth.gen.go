...
func (p *RoleAuth) fieldPtr(args ...dialect.Field) []func(*RoleAuth) any {
	if len(args) == 0 {
		args = tblroleauth.ReadableFields
	}
	fs := make([]func(*RoleAuth) any, 0, len(args))
	for _, col := range args {
		if f, ok := roleauthFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *RoleAuth) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := roleauthFieldToPtrFunc[col]; ok {
...
func (p *RoleAuth) Slice(rows *sql.Rows, args ...dialect.Field) ([]*RoleAuth, bool, error) {
	defer rows.Close()
...
func (p *RoleAuth) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := roleauthFieldToValueFunc[arg]; has {
			value, isZero := f(p)
