...
func (p *MedicineSet) fieldPtr(args ...dialect.Field) []func(*MedicineSet) any {
	if len(args) == 0 {
		args = tblmedicineset.ReadableFields
	}
	fs := make([]func(*MedicineSet) any, 0, len(args))
	for _, col := range args {
		if f, ok := medicinesetFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *MedicineSet) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := medicinesetFieldToPtrFunc[col]; ok {
...
func (p *MedicineSet) Slice(rows *sql.Rows, args ...dialect.Field) ([]*MedicineSet, bool, error) {
	defer rows.Close()
...
func (p *MedicineSet) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := medicinesetFieldToValueFunc[arg]; has {
			value, isZero := f(p)
