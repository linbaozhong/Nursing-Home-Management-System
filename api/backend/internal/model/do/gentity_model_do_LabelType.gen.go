...
func (p *LabelType) fieldPtr(args ...dialect.Field) []func(*LabelType) any {
	if len(args) == 0 {
		args = tbllabeltype.ReadableFields
	}
	fs := make([]func(*LabelType) any, 0, len(args))
	for _, col := range args {
		if f, ok := labeltypeFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *LabelType) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := labeltypeFieldToPtrFunc[col]; ok {
...
func (p *LabelType) Slice(rows *sql.Rows, args ...dialect.Field) ([]*LabelType, bool, error) {
	defer rows.Close()
...
func (p *LabelType) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := labeltypeFieldToValueFunc[arg]; has {
			value, isZero := f(p)
