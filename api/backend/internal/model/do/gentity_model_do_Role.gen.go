...
func (p *Role) fieldPtr(args ...dialect.Field) []func(*Role) any {
	if len(args) == 0 {
		args = tblrole.ReadableFields
	}
	fs := make([]func(*Role) any, 0, len(args))
	for _, col := range args {
		if f, ok := roleFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Role) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := roleFieldToPtrFunc[col]; ok {
...
func (p *Role) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Role, bool, error) {
	defer rows.Close()
	roles := make([]*Role, 0)
...
func (p *Role) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := roleFieldToValueFunc[arg]; has {
			value, isZero := f(p)
