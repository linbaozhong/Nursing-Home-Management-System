// Code generated by github.com/linbaozhong/gentity. DO NOT EDIT.

package do

import (
	"api/internal/model/define/table/tblmedicine"
	"database/sql"
	"errors"
	"github.com/linbaozhong/gentity/pkg/ace/dialect"
	"github.com/linbaozhong/gentity/pkg/ace/pool"
	"github.com/linbaozhong/gentity/pkg/gjson"
	"github.com/linbaozhong/gentity/pkg/log"
	"github.com/linbaozhong/gentity/pkg/types"
)

const MedicineTableName = "medicine"

var (
	medicinePool = pool.New[*Medicine](func() any {
		_obj := &Medicine{}
		return _obj
	})
)

func NewMedicine() *Medicine {
	_obj := medicinePool.Get()
	return _obj
}

// MarshalJSON
func (p *Medicine) MarshalJSON() ([]byte, error) {
	write := types.NewJsonWriter(11 * 50)
	if p.Name != "" {
		write.WriteRaw("name", types.Marshal(p.Name))
	}
	if p.Type != "" {
		write.WriteRaw("type", types.Marshal(p.Type))
	}
	if p.Specification != "" {
		write.WriteRaw("specification", types.Marshal(p.Specification))
	}
	if p.DosageForm != "" {
		write.WriteRaw("dosage_form", types.Marshal(p.DosageForm))
	}
	if p.Manufacturer != "" {
		write.WriteRaw("manufacturer", types.Marshal(p.Manufacturer))
	}
	if p.DelFlag != "" {
		write.WriteRaw("del_flag", types.Marshal(p.DelFlag))
	}
	if p.Id != 0 {
		write.WriteRaw("id", types.Marshal(p.Id))
	}
	if p.CreateId != 0 {
		write.WriteRaw("create_id", types.Marshal(p.CreateId))
	}
	if !p.CreateTime.IsZero() {
		write.WriteRaw("create_time", types.Marshal(p.CreateTime))
	}
	if p.UpdateId != 0 {
		write.WriteRaw("update_id", types.Marshal(p.UpdateId))
	}
	if !p.UpdateTime.IsZero() {
		write.WriteRaw("update_time", types.Marshal(p.UpdateTime))
	}
	return write.Bytes(), nil
}

// UnmarshalJSON
func (p *Medicine) UnmarshalJSON(data []byte) error {

	if !gjson.ValidBytes(data) {
		return errors.New("invalid json")
	}
	_result := gjson.ParseBytes(data)
	_result.ForEach(func(key, value gjson.Result) bool {
		var e error
		switch key.Str {
		case "name":
			p.Name = types.String(value.Str)
		case "type":
			p.Type = types.String(value.Str)
		case "specification":
			p.Specification = types.String(value.Str)
		case "dosage_form":
			p.DosageForm = types.String(value.Str)
		case "manufacturer":
			p.Manufacturer = types.String(value.Str)
		case "del_flag":
			p.DelFlag = types.String(value.Str)
		case "id":
			p.Id = types.BigInt(value.Uint())
		case "create_id":
			p.CreateId = types.BigInt(value.Uint())
		case "create_time":
			p.CreateTime = types.Time{Time: value.Time()}
		case "update_id":
			p.UpdateId = types.BigInt(value.Uint())
		case "update_time":
			p.UpdateTime = types.Time{Time: value.Time()}
		}
		if e != nil {
			log.Error(e)
			return false
		}
		return true
	})
	return nil
}

// Free
func (p *Medicine) Free() {
	if p == nil {
		return
	}

	medicinePool.Put(p)
}

// Reset
func (p *Medicine) Reset() {
	p.Name = ""
	p.Type = ""
	p.Specification = ""
	p.DosageForm = ""
	p.Manufacturer = ""
	p.DelFlag = ""
	p.Id = 0
	p.CreateId = 0
	p.CreateTime = types.Time{}
	p.UpdateId = 0
	p.UpdateTime = types.Time{}

}

func (p *Medicine) TableName() string {
	return MedicineTableName
}

// 定义一个映射表，将字段与对应的指针获取函数关联
var medicineFieldToPtrFunc = map[dialect.Field]func(*Medicine) any{
	tblmedicine.Name:          func(p *Medicine) any { return &p.Name },
	tblmedicine.Type:          func(p *Medicine) any { return &p.Type },
	tblmedicine.Specification: func(p *Medicine) any { return &p.Specification },
	tblmedicine.DosageForm:    func(p *Medicine) any { return &p.DosageForm },
	tblmedicine.Manufacturer:  func(p *Medicine) any { return &p.Manufacturer },
	tblmedicine.DelFlag:       func(p *Medicine) any { return &p.DelFlag },
	tblmedicine.Id:            func(p *Medicine) any { return &p.Id },
	tblmedicine.CreateId:      func(p *Medicine) any { return &p.CreateId },
	tblmedicine.CreateTime:    func(p *Medicine) any { return &p.CreateTime },
	tblmedicine.UpdateId:      func(p *Medicine) any { return &p.UpdateId },
	tblmedicine.UpdateTime:    func(p *Medicine) any { return &p.UpdateTime },
}

// AssignPtr 根据传入的字段参数，返回对应字段的指针切片。
// 如果未传入任何字段参数，则默认使用 ReadableFields 中的字段。
// 参数 args 为可变参数，代表需要获取指针的字段。
// 返回值为一个包含对应字段指针的切片。
func (p *Medicine) AssignPtr(args ...dialect.Field) []any {
	if len(args) == 0 {
		args = tblmedicine.ReadableFields
	}

	_vals := make([]any, 0, len(args))
	for _, col := range args {
		if ptrFunc, ok := medicineFieldToPtrFunc[col]; ok {
			_vals = append(_vals, ptrFunc(p))
		}
	}

	return _vals
}

func (p *Medicine) Scan(rows *sql.Rows, args ...dialect.Field) ([]*Medicine, bool, error) {
	defer rows.Close()
	medicines := make([]*Medicine, 0)

	if len(args) == 0 {
		args = tblmedicine.ReadableFields
	}

	for rows.Next() {
		_p := NewMedicine()
		_vals := _p.AssignPtr(args...)
		e := rows.Scan(_vals...)
		if e != nil {
			log.Error(e)
			return nil, false, e
		}
		medicines = append(medicines, _p)
	}
	if e := rows.Err(); e != nil {
		log.Error(e)
		return nil, false, e
	}
	return medicines, len(medicines) > 0, nil
}

// RawAssignValues 向数据库写入数据前，为表列赋值。多用于批量插入和更新
// 如果 args 为空，则赋值所有可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Medicine) RawAssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	if len(args) == 0 {
		args = tblmedicine.WritableFields
	}
	return p.AssignValues(d, args...)
}

// 定义字段到值检查和获取函数的映射
var medicineFieldToValueFunc = map[dialect.Field]func(*Medicine) (any, bool){
	tblmedicine.Name: func(p *Medicine) (any, bool) {
		return p.Name, p.Name == ""
	},
	tblmedicine.Type: func(p *Medicine) (any, bool) {
		return p.Type, p.Type == ""
	},
	tblmedicine.Specification: func(p *Medicine) (any, bool) {
		return p.Specification, p.Specification == ""
	},
	tblmedicine.DosageForm: func(p *Medicine) (any, bool) {
		return p.DosageForm, p.DosageForm == ""
	},
	tblmedicine.Manufacturer: func(p *Medicine) (any, bool) {
		return p.Manufacturer, p.Manufacturer == ""
	},
	tblmedicine.DelFlag: func(p *Medicine) (any, bool) {
		return p.DelFlag, p.DelFlag == ""
	},
	tblmedicine.Id: func(p *Medicine) (any, bool) {
		return p.Id, p.Id == 0
	},
	tblmedicine.CreateId: func(p *Medicine) (any, bool) {
		return p.CreateId, p.CreateId == 0
	},
	tblmedicine.CreateTime: func(p *Medicine) (any, bool) {
		return p.CreateTime, p.CreateTime.IsZero()
	},
	tblmedicine.UpdateId: func(p *Medicine) (any, bool) {
		return p.UpdateId, p.UpdateId == 0
	},
	tblmedicine.UpdateTime: func(p *Medicine) (any, bool) {
		return p.UpdateTime, p.UpdateTime.IsZero()
	},
}

// AssignValues 向数据库写入数据前，为表列赋值。
// 如果 args 为空，则将非零值赋与可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Medicine) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	// 未传参时使用全部可写字段，并跳过零值
	skipZero := len(args) == 0
	if skipZero {
		args = tblmedicine.WritableFields
	}

	cols := make([]string, 0, len(args))
	vals := make([]any, 0, len(args))

	for _, arg := range args {
		if valueFunc, exists := medicineFieldToValueFunc[arg]; exists {
			value, isZero := valueFunc(p)
			// 显式指定字段时全量包含；默认模式跳过零值字段
			if skipZero && isZero {
				continue
			}
			cols = append(cols, arg.Quote(d))
			vals = append(vals, value)
		}
	}
	return cols, vals
}

//
func (p *Medicine) AssignKeys() (dialect.Field, any) {
	return tblmedicine.PrimaryKey, p.Id
}

//
func (p *Medicine) AssignPrimaryKeyValues(result sql.Result) error {
	return nil
}
