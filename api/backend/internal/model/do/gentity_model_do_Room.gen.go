...
func (p *Room) TableName() string {
	return RoomTableName
}

// 定义一个映射表，将字段与对应的指针获取函数关联
var roomFieldToPtrFunc = map[dialect.Field]func(*Room) any{
	tblroom.Name:       func(p *Room) any { return &p.Name },
	tblroom.DelFlag:    func(p *Room) any { return &p.DelFlag },
	tblroom.Id:         func(p *Room) any { return &p.Id },
	tblroom.TypeId:     func(p *Room) any { return &p.TypeId },
	tblroom.FloorId:    func(p *Room) any { return &p.FloorId },
	tblroom.CreateId:   func(p *Room) any { return &p.CreateId },
	tblroom.CreateTime: func(p *Room) any { return &p.CreateTime },
	tblroom.UpdateId:   func(p *Room) any { return &p.UpdateId },
	tblroom.UpdateTime: func(p *Room) any { return &p.UpdateTime },
	tblroom.BedNum:     func(p *Room) any { return &p.BedNum },
}
...
// 返回值为一个包含对应字段指针的切片。
func (p *Room) AssignPtr(args ...dialect.Field) []any {
	if len(args) == 0 {
		args = tblroom.ReadableFields
	}

	_vals := make([]any, 0, len(args))
	for _, col := range args {
		if ptrFunc, ok := roomFieldToPtrFunc[col]; ok {
...
func (p *Room) Scan(rows *sql.Rows, args ...dialect.Field) ([]*Room, bool, error) {
	defer rows.Close()
	rooms := make([]*Room, 0)
	...
	for rows.Next() {
		...
		rooms = append(rooms, _p)
	}
	if e := rows.Err(); e != nil {
		log.Error(e)
		return nil, false, e
	}
	return rooms, len(rooms) > 0, nil
}

// RawAssignValues 向数据库写入数据前，为表列赋值。多用于批量插入和更新
// 如果 args 为空，则赋值所有可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Room) RawAssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	if len(args) == 0 {
		args = tblroom.WritableFields
	}
	return p.AssignValues(d, args...)
}

// 定义字段到值检查和获取函数的映射
var roomFieldToValueFunc = map[dialect.Field]func(*Room) (any, bool){
	...
	tblroom.UpdateTime: func(p *Room) (any, bool) {
...
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Room) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	// 未传参时使用全部可写字段，并跳过零值
	skipZero := len(args) == 0
	if skipZero {
		args = tblroom.WritableFields
	...
	for _, arg := range args {
		if valueFunc, exists := roomFieldToValueFunc[arg]; exists {
...
//
func (p *Room) AssignKeys() (dialect.Field, any) {
	return tblroom.PrimaryKey, p.Id
}

//
func (p *Room) AssignPrimaryKeyValues(result sql.Result) error {
	return nil
}
