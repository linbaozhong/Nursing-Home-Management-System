...
func (p *Material) TableName() string {
	return "material"
}
...
func (p *Material) fieldPtr(args ...dialect.Field) []func(*Material) any {
	if len(args) == 0 {
		args = tblmaterial.ReadableFields
	}
	fs := make([]func(*Material) any, 0, len(args))
	for _, col := range args {
		if f, ok := materialFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
// 返回值为一个包含对应字段指针的切片。
func (p *Material) AssignPtr(args ...dialect.Field) []any {
	fs := p.fieldPtr(args...)
...
func (p *Material) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := materialFieldToPtrFunc[col]; ok {
...
func (p *Material) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Material, bool, error) {
	defer rows.Close()
	...
	for rows.Next() {
		...
		materials = append(materials, _p)
	}
	if e := rows.Err(); e != nil {
		log.Error(e)
		return nil, false, e
	}
	return materials, len(materials) > 0, nil
}

// RawAssignValues 向数据库写入数据前，为表列赋值。多用于批量插入和更新
// 如果 args 为空，则赋值所有可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Material) RawAssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	if len(args) == 0 {
		args = tblmaterial.WritableFields
	}
	return p.AssignValues(d, args...)
}

// 定义字段到值检查和获取函数的映射
var materialFieldToValueFunc = map[dialect.Field]func(*Material) (any, bool){
...
// 如果 args 不为空，则只赋值 args 中的字段
func (p *Material) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	// 未传参时使用全部可写字段，并跳过零值
	skipZero := len(args) == 0
	if skipZero {
		args = tblmaterial.WritableFields
	...
	for _, arg := range args {
		if f, has := materialFieldToValueFunc[arg]; has {
			value, isZero := f(p)
