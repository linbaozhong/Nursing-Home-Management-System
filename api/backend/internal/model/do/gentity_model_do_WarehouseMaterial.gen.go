// Code generated by github.com/linbaozhong/gentity. DO NOT EDIT.

package do

import (
	"api/internal/model/define/table/tblwarehousematerial"
	"database/sql"
	"errors"
	"github.com/linbaozhong/gentity/pkg/ace/dialect"
	"github.com/linbaozhong/gentity/pkg/ace/pool"
	"github.com/linbaozhong/gentity/pkg/gjson"
	"github.com/linbaozhong/gentity/pkg/log"
	"github.com/linbaozhong/gentity/pkg/types"
)

const WarehouseMaterialTableName = "warehouse_material"

var (
	warehousematerialPool = pool.New[*WarehouseMaterial](func() any {
		_obj := &WarehouseMaterial{}
		return _obj
	})
)

func NewWarehouseMaterial() *WarehouseMaterial {
	_obj := warehousematerialPool.Get()
	return _obj
}

// MarshalJSON
func (p *WarehouseMaterial) MarshalJSON() ([]byte, error) {
	write := types.NewJsonWriter(11 * 50)
	if p.Id != 0 {
		write.WriteRaw("id", types.Marshal(p.Id))
	}
	if p.WarehouseRecordId != 0 {
		write.WriteRaw("warehouse_record_id", types.Marshal(p.WarehouseRecordId))
	}
	if p.MaterialId != 0 {
		write.WriteRaw("material_id", types.Marshal(p.MaterialId))
	}
	if !p.ProductDate.IsZero() {
		write.WriteRaw("product_date", types.Marshal(p.ProductDate))
	}
	if !p.ExpireDate.IsZero() {
		write.WriteRaw("expire_date", types.Marshal(p.ExpireDate))
	}
	if p.CreateId != 0 {
		write.WriteRaw("create_id", types.Marshal(p.CreateId))
	}
	if !p.CreateTime.IsZero() {
		write.WriteRaw("create_time", types.Marshal(p.CreateTime))
	}
	if p.UpdateId != 0 {
		write.WriteRaw("update_id", types.Marshal(p.UpdateId))
	}
	if !p.UpdateTime.IsZero() {
		write.WriteRaw("update_time", types.Marshal(p.UpdateTime))
	}
	if p.WarehouseNum != 0 {
		write.WriteRaw("warehouse_num", types.Marshal(p.WarehouseNum))
	}
	if p.Inventory != 0 {
		write.WriteRaw("inventory", types.Marshal(p.Inventory))
	}
	return write.Bytes(), nil
}

// UnmarshalJSON
func (p *WarehouseMaterial) UnmarshalJSON(data []byte) error {

	if !gjson.ValidBytes(data) {
		return errors.New("invalid json")
	}
	_result := gjson.ParseBytes(data)
	_result.ForEach(func(key, value gjson.Result) bool {
		var e error
		switch key.Str {
		case "id":
			p.Id = types.BigInt(value.Uint())
		case "warehouse_record_id":
			p.WarehouseRecordId = types.BigInt(value.Uint())
		case "material_id":
			p.MaterialId = types.BigInt(value.Uint())
		case "product_date":
			p.ProductDate = types.Time{Time: value.Time()}
		case "expire_date":
			p.ExpireDate = types.Time{Time: value.Time()}
		case "create_id":
			p.CreateId = types.BigInt(value.Uint())
		case "create_time":
			p.CreateTime = types.Time{Time: value.Time()}
		case "update_id":
			p.UpdateId = types.BigInt(value.Uint())
		case "update_time":
			p.UpdateTime = types.Time{Time: value.Time()}
		case "warehouse_num":
			p.WarehouseNum = types.Int32(value.Int())
		case "inventory":
			p.Inventory = types.Int32(value.Int())
		}
		if e != nil {
			log.Error(e)
			return false
		}
		return true
	})
	return nil
}

// Free
func (p *WarehouseMaterial) Free() {
	if p == nil {
		return
	}

	warehousematerialPool.Put(p)
}

// Reset
func (p *WarehouseMaterial) Reset() {
	p.Id = 0
	p.WarehouseRecordId = 0
	p.MaterialId = 0
	p.ProductDate = types.Time{}
	p.ExpireDate = types.Time{}
	p.CreateId = 0
	p.CreateTime = types.Time{}
	p.UpdateId = 0
	p.UpdateTime = types.Time{}
	p.WarehouseNum = 0
	p.Inventory = 0

}

func (p *WarehouseMaterial) TableName() string {
	return WarehouseMaterialTableName
}

// 定义一个映射表，将字段与对应的指针获取函数关联
var warehousematerialFieldToPtrFunc = map[dialect.Field]func(*WarehouseMaterial) any{
	tblwarehousematerial.Id:                func(p *WarehouseMaterial) any { return &p.Id },
	tblwarehousematerial.WarehouseRecordId: func(p *WarehouseMaterial) any { return &p.WarehouseRecordId },
	tblwarehousematerial.MaterialId:        func(p *WarehouseMaterial) any { return &p.MaterialId },
	tblwarehousematerial.ProductDate:       func(p *WarehouseMaterial) any { return &p.ProductDate },
	tblwarehousematerial.ExpireDate:        func(p *WarehouseMaterial) any { return &p.ExpireDate },
	tblwarehousematerial.CreateId:          func(p *WarehouseMaterial) any { return &p.CreateId },
	tblwarehousematerial.CreateTime:        func(p *WarehouseMaterial) any { return &p.CreateTime },
	tblwarehousematerial.UpdateId:          func(p *WarehouseMaterial) any { return &p.UpdateId },
	tblwarehousematerial.UpdateTime:        func(p *WarehouseMaterial) any { return &p.UpdateTime },
	tblwarehousematerial.WarehouseNum:      func(p *WarehouseMaterial) any { return &p.WarehouseNum },
	tblwarehousematerial.Inventory:         func(p *WarehouseMaterial) any { return &p.Inventory },
}

// AssignPtr 根据传入的字段参数，返回对应字段的指针切片。
// 如果未传入任何字段参数，则默认使用 ReadableFields 中的字段。
// 参数 args 为可变参数，代表需要获取指针的字段。
// 返回值为一个包含对应字段指针的切片。
func (p *WarehouseMaterial) AssignPtr(args ...dialect.Field) []any {
	if len(args) == 0 {
		args = tblwarehousematerial.ReadableFields
	}

	_vals := make([]any, 0, len(args))
	for _, col := range args {
		if ptrFunc, ok := warehousematerialFieldToPtrFunc[col]; ok {
			_vals = append(_vals, ptrFunc(p))
		}
	}

	return _vals
}

func (p *WarehouseMaterial) Scan(rows *sql.Rows, args ...dialect.Field) ([]*WarehouseMaterial, bool, error) {
	defer rows.Close()
	warehouse_materials := make([]*WarehouseMaterial, 0)

	if len(args) == 0 {
		args = tblwarehousematerial.ReadableFields
	}

	for rows.Next() {
		_p := NewWarehouseMaterial()
		_vals := _p.AssignPtr(args...)
		e := rows.Scan(_vals...)
		if e != nil {
			log.Error(e)
			return nil, false, e
		}
		warehouse_materials = append(warehouse_materials, _p)
	}
	if e := rows.Err(); e != nil {
		log.Error(e)
		return nil, false, e
	}
	return warehouse_materials, len(warehouse_materials) > 0, nil
}

// RawAssignValues 向数据库写入数据前，为表列赋值。多用于批量插入和更新
// 如果 args 为空，则赋值所有可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *WarehouseMaterial) RawAssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	if len(args) == 0 {
		args = tblwarehousematerial.WritableFields
	}
	return p.AssignValues(d, args...)
}

// 定义字段到值检查和获取函数的映射
var warehousematerialFieldToValueFunc = map[dialect.Field]func(*WarehouseMaterial) (any, bool){
	tblwarehousematerial.Id: func(p *WarehouseMaterial) (any, bool) {
		return p.Id, p.Id == 0
	},
	tblwarehousematerial.WarehouseRecordId: func(p *WarehouseMaterial) (any, bool) {
		return p.WarehouseRecordId, p.WarehouseRecordId == 0
	},
	tblwarehousematerial.MaterialId: func(p *WarehouseMaterial) (any, bool) {
		return p.MaterialId, p.MaterialId == 0
	},
	tblwarehousematerial.ProductDate: func(p *WarehouseMaterial) (any, bool) {
		return p.ProductDate, p.ProductDate.IsZero()
	},
	tblwarehousematerial.ExpireDate: func(p *WarehouseMaterial) (any, bool) {
		return p.ExpireDate, p.ExpireDate.IsZero()
	},
	tblwarehousematerial.CreateId: func(p *WarehouseMaterial) (any, bool) {
		return p.CreateId, p.CreateId == 0
	},
	tblwarehousematerial.CreateTime: func(p *WarehouseMaterial) (any, bool) {
		return p.CreateTime, p.CreateTime.IsZero()
	},
	tblwarehousematerial.UpdateId: func(p *WarehouseMaterial) (any, bool) {
		return p.UpdateId, p.UpdateId == 0
	},
	tblwarehousematerial.UpdateTime: func(p *WarehouseMaterial) (any, bool) {
		return p.UpdateTime, p.UpdateTime.IsZero()
	},
	tblwarehousematerial.WarehouseNum: func(p *WarehouseMaterial) (any, bool) {
		return p.WarehouseNum, p.WarehouseNum == 0
	},
	tblwarehousematerial.Inventory: func(p *WarehouseMaterial) (any, bool) {
		return p.Inventory, p.Inventory == 0
	},
}

// AssignValues 向数据库写入数据前，为表列赋值。
// 如果 args 为空，则将非零值赋与可写字段
// 如果 args 不为空，则只赋值 args 中的字段
func (p *WarehouseMaterial) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	// 未传参时使用全部可写字段，并跳过零值
	skipZero := len(args) == 0
	if skipZero {
		args = tblwarehousematerial.WritableFields
	}

	cols := make([]string, 0, len(args))
	vals := make([]any, 0, len(args))

	for _, arg := range args {
		if valueFunc, exists := warehousematerialFieldToValueFunc[arg]; exists {
			value, isZero := valueFunc(p)
			// 显式指定字段时全量包含；默认模式跳过零值字段
			if skipZero && isZero {
				continue
			}
			cols = append(cols, arg.Quote(d))
			vals = append(vals, value)
		}
	}
	return cols, vals
}

//
func (p *WarehouseMaterial) AssignKeys() (dialect.Field, any) {
	return tblwarehousematerial.PrimaryKey, p.Id
}

//
func (p *WarehouseMaterial) AssignPrimaryKeyValues(result sql.Result) error {
	return nil
}
