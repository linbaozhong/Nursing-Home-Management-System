...
type PageNurseGradeByKeyQuery struct {
	...
	GradeName *string `json:"gradeName"` // 级别名称
	NurseType *string `json:"nurseType"` // 护理类型
}
...
type PageNurseGradeByKeyVO struct {
	...
	MonthPrice float64 `json:"monthPrice"` // 月护理费用
}
...
type PageNurseByKeyQuery struct {
	PageNum   *int    `json:"pageNum" valid:"required"` // 页码
	PageSize  *int    `json:"pageSize" valid:"required"` // 条数
	NurseName *string `json:"nurseName"` // 护理员姓名
...
type AddNurseQuery struct {
	ID       *int64  `json:"id"` // id
	NurseName *string `json:"nurseName" valid:"required"` // 护理员姓名
	Phone    *string `json:"phone" valid:"required"` // 电话
	GradeID  *int64  `json:"gradeId" valid:"required"` // 护理等级编号
}
