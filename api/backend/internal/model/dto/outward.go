...
type AddOutwardQuery struct {
	...
	OutwardDate    *string `json:"outward_date" valid:"required"`     // 外出时间
	PlanReturnDate *string `json:"plan_return_date" valid:"required"` // 计划返回时间
}
...
type RecordReturnQuery struct {
	ID             *int64  `json:"id" valid:"required"`               // id
	RealReturnDate *string `json:"real_return_date" valid:"required"` // 实际返回时间
}
