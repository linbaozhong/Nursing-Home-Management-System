...
// 如果不传入，则使用默认的db
func HealthInfo(x ace.Executer) *healthinfo {
...
// Debug 仅打印SQL语句，不执行
func (p *healthinfo) Debug() *healthinfo {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *healthinfo) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *healthinfo) InsertBatch(ctx context.Context, beans []*do.HealthInfo, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *healthinfo) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *healthinfo) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblhealthinfo.PrimaryKey.Eq(id),
	)
}
...
func (p *healthinfo) UpdateBatch(ctx context.Context, beans []*do.HealthInfo, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *healthinfo) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblhealthinfo.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *healthinfo) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *healthinfo) Get(ctx context.Context, s ace.SelectBuilder) (*do.HealthInfo, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblhealthinfo.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *healthinfo) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.HealthInfo, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblhealthinfo.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *healthinfo) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblhealthinfo.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *healthinfo) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblhealthinfo.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
