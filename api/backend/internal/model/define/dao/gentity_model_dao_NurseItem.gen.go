...
// 如果不传入，则使用默认的db
func NurseItem(x ace.Executer) *nurseitem {
...
// Debug 仅打印SQL语句，不执行
func (p *nurseitem) Debug() *nurseitem {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *nurseitem) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *nurseitem) InsertBatch(ctx context.Context, beans []*do.NurseItem, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *nurseitem) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
...
// UpdateById
func (p *nurseitem) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblnurseitem.PrimaryKey.Eq(id),
	)
}
...
func (p *nurseitem) UpdateBatch(ctx context.Context, beans []*do.NurseItem, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *nurseitem) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblnurseitem.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *nurseitem) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *nurseitem) Get(ctx context.Context, s ace.SelectBuilder) (*do.NurseItem, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblnurseitem.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *nurseitem) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.NurseItem, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblnurseitem.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *nurseitem) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblnurseitem.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *nurseitem) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblnurseitem.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
