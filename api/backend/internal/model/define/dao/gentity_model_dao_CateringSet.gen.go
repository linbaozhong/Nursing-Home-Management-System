...
// 如果不传入，则使用默认的db
func CateringSet(x ace.Executer) *cateringset {
...
// Debug 仅打印SQL语句，不执行
func (p *cateringset) Debug() *cateringset {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *cateringset) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *cateringset) InsertBatch(ctx context.Context, beans []*do.CateringSet, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *cateringset) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
...
// UpdateById
func (p *cateringset) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblcateringset.PrimaryKey.Eq(id),
	)
}
...
func (p *cateringset) UpdateBatch(ctx context.Context, beans []*do.CateringSet, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *cateringset) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblcateringset.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *cateringset) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *cateringset) Get(ctx context.Context, s ace.SelectBuilder) (*do.CateringSet, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblcateringset.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *cateringset) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.CateringSet, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblcateringset.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *cateringset) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblcateringset.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *cateringset) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblcateringset.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
