...
// New 实例化medicinerecord
// exec: 执行器，用于执行sql语句，如：db,tx
// 如果不传入，则使用默认的db
func MedicineRecord(x ace.Executer) *medicinerecord {
...
// Debug 仅打印SQL语句，不执行
func (p *medicinerecord) Debug() *medicinerecord {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *medicinerecord) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *medicinerecord) InsertBatch(ctx context.Context, beans []*do.MedicineRecord, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *medicinerecord) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *medicinerecord) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblmedicinerecord.PrimaryKey.Eq(id),
	)
}
...
func (p *medicinerecord) UpdateBatch(ctx context.Context, beans []*do.MedicineRecord, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *medicinerecord) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblmedicinerecord.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *medicinerecord) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
//
// 返回值:
//  1. *do.MedicineRecord: 第一个符合条件的记录
//  2. bool: 是否存在记录
//  3. error: 错误信息
//
// 注意:
...
func (p *medicinerecord) Get(ctx context.Context, s ace.SelectBuilder) (*do.MedicineRecord, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblmedicinerecord.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *medicinerecord) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.MedicineRecord, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblmedicinerecord.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *medicinerecord) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblmedicinerecord.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *medicinerecord) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblmedicinerecord.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
