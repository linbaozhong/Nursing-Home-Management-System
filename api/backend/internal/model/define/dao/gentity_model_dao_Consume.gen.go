...
// 如果不传入，则使用默认的db
func Consume(x ace.Executer) *consume {
...
// Debug 仅打印SQL语句，不执行
func (p *consume) Debug() *consume {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *consume) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *consume) InsertBatch(ctx context.Context, beans []*do.Consume, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *consume) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *consume) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblconsume.PrimaryKey.Eq(id),
	)
}
...
func (p *consume) UpdateBatch(ctx context.Context, beans []*do.Consume, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *consume) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblconsume.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *consume) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *consume) Get(ctx context.Context, s ace.SelectBuilder) (*do.Consume, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblconsume.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *consume) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.Consume, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblconsume.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *consume) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblconsume.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
