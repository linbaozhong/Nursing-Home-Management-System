...
// 如果不传入，则使用默认的db
func Warehouse(x ace.Executer) *warehouse {
...
// Debug 仅打印SQL语句，不执行
func (p *warehouse) Debug() *warehouse {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *warehouse) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *warehouse) InsertBatch(ctx context.Context, beans []*do.Warehouse, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *warehouse) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *warehouse) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblwarehouse.PrimaryKey.Eq(id),
	)
}
...
func (p *warehouse) UpdateBatch(ctx context.Context, beans []*do.Warehouse, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *warehouse) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblwarehouse.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *warehouse) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *warehouse) Get(ctx context.Context, s ace.SelectBuilder) (*do.Warehouse, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblwarehouse.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *warehouse) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.Warehouse, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblwarehouse.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *warehouse) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblwarehouse.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *warehouse) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblwarehouse.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
