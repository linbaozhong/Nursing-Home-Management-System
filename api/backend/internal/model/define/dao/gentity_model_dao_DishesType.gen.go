...
// 如果不传入，则使用默认的db
func DishesType(x ace.Executer) *dishestype {
...
// Debug 仅打印SQL语句，不执行
func (p *dishestype) Debug() *dishestype {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *dishestype) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *dishestype) InsertBatch(ctx context.Context, beans []*do.DishesType, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *dishestype) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
...
// UpdateById
func (p *dishestype) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tbldishestype.PrimaryKey.Eq(id),
	)
}
...
func (p *dishestype) UpdateBatch(ctx context.Context, beans []*do.DishesType, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *dishestype) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tbldishestype.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *dishestype) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *dishestype) Get(ctx context.Context, s ace.SelectBuilder) (*do.DishesType, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tbldishestype.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *dishestype) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.DishesType, error) {
	_objs, _, e := p.List(ctx, ace.Where(tbldishestype.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *dishestype) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tbldishestype.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *dishestype) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tbldishestype.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
