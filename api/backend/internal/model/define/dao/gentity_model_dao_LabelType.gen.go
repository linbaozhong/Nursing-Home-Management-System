...
// 如果不传入，则使用默认的db
func LabelType(x ace.Executer) *labeltype {
...
// Debug 仅打印SQL语句，不执行
func (p *labeltype) Debug() *labeltype {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *labeltype) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *labeltype) InsertBatch(ctx context.Context, beans []*do.LabelType, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *labeltype) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *labeltype) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tbllabeltype.PrimaryKey.Eq(id),
	)
}
...
func (p *labeltype) UpdateBatch(ctx context.Context, beans []*do.LabelType, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *labeltype) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tbllabeltype.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *labeltype) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *labeltype) Get(ctx context.Context, s ace.SelectBuilder) (*do.LabelType, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tbllabeltype.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *labeltype) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.LabelType, error) {
	_objs, _, e := p.List(ctx, ace.Where(tbllabeltype.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *labeltype) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tbllabeltype.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *labeltype) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tbllabeltype.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
