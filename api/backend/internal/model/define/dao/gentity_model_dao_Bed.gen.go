...
// Debug 仅打印SQL语句，不执行
func (p *bed) Debug() *bed {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *bed) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *bed) InsertBatch(ctx context.Context, beans []*do.Bed, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *bed) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *bed) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblbed.PrimaryKey.Eq(id),
	)
}
...
func (p *bed) UpdateBatch(ctx context.Context, beans []*do.Bed, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *bed) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblbed.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *bed) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *bed) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.Bed, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblbed.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
