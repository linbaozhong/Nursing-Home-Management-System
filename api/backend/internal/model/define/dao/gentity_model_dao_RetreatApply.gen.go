...
// 如果不传入，则使用默认的db
func RetreatApply(x ace.Executer) *retreatapply {
...
// Debug 仅打印SQL语句，不执行
func (p *retreatapply) Debug() *retreatapply {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *retreatapply) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *retreatapply) InsertBatch(ctx context.Context, beans []*do.RetreatApply, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *retreatapply) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *retreatapply) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblretreatapply.PrimaryKey.Eq(id),
	)
}
...
func (p *retreatapply) UpdateBatch(ctx context.Context, beans []*do.RetreatApply, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *retreatapply) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblretreatapply.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *retreatapply) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *retreatapply) Get(ctx context.Context, s ace.SelectBuilder) (*do.RetreatApply, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblretreatapply.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *retreatapply) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.RetreatApply, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblretreatapply.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *retreatapply) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblretreatapply.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *retreatapply) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblretreatapply.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
