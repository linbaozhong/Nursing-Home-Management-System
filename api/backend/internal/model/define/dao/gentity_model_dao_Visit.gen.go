...
// 如果不传入，则使用默认的db
func Visit(x ace.Executer) *visit {
	_obj := visitPool.Get()
	_obj.x = x
	return _obj
}
...
// Debug 仅打印SQL语句，不执行
func (p *visit) Debug() *visit {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *visit) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *visit) InsertBatch(ctx context.Context, beans []*do.Visit, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *visit) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
...
// UpdateById
func (p *visit) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblvisit.PrimaryKey.Eq(id),
	)
}
...
func (p *visit) UpdateBatch(ctx context.Context, beans []*do.Visit, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *visit) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblvisit.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *visit) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *visit) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.Visit, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblvisit.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *visit) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblvisit.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
