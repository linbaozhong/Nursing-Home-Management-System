...
// 如果不传入，则使用默认的db
func NurseReserve(x ace.Executer) *nursereserve {
...
// Debug 仅打印SQL语句，不执行
func (p *nursereserve) Debug() *nursereserve {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *nursereserve) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
...
func (p *nursereserve) InsertBatch(ctx context.Context, beans []*do.NurseReserve, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *nursereserve) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
		return true, nil
	}
...
// UpdateById
func (p *nursereserve) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
		tblnursereserve.PrimaryKey.Eq(id),
	)
}
...
func (p *nursereserve) UpdateBatch(ctx context.Context, beans []*do.NurseReserve, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *nursereserve) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblnursereserve.PrimaryKey.Eq(id),
	)
}
...
// ids: 主键列表
// 返回值: 影响的行数，错误信息
func (p *nursereserve) DeleteByIds(ctx context.Context, ids []any) (int64, error) {
	defer p.Free()
	if len(ids) == 0 {
		return 0, nil
	}
...
func (p *nursereserve) Get(ctx context.Context, s ace.SelectBuilder) (*do.NurseReserve, bool, error) {
	...
	l := len(_cols)
	if l == 0 {
		_cols = tblnursereserve.ReadableFields
		s.Cols(_cols...)
	}
	if s.GetDB() == nil {
		s.SetDB(p.x)
...
func (p *nursereserve) GetByIds(ctx context.Context, ids []any, cols ...dialect.Field) ([]*do.NurseReserve, error) {
	_objs, _, e := p.List(ctx, ace.Where(tblnursereserve.PrimaryKey.In(ids...)).Cols(cols...))
	return _objs, e
}
...
func (p *nursereserve) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.Cols(tblnursereserve.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
...
func (p *nursereserve) Column(ctx context.Context, s ace.SelectBuilder) ([]any, error) {
	...
	_cols := s.GetCols()
	if len(_cols) == 0 {
		s.PureCols(tblnursereserve.PrimaryKey)
	} else {
		s.PureCols(_cols[0])
	}
