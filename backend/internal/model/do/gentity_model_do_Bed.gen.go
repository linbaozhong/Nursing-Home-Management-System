...
func (p *Bed) UnmarshalJSON(data []byte) error {
	...
	_result.ForEach(func(key, value gjson.Result) bool {
		...
		case "bed_type_id":
			p.BedTypeID = types.BigInt(value.Uint())
		case "create_id":
...
var bedFieldToPtrFunc = map[string]func(*Bed) any{
	...
	tblbed.BedTypeID.Name:  func(p *Bed) any { return &p.BedTypeID },
...
func (p *Bed) fieldPtr(args ...dialect.Field) []func(*Bed) any {
	if len(args) == 0 {
		args = tblbed.ReadableFields
	}
	fs := make([]func(*Bed) any, 0, len(args))
	for _, col := range args {
		if f, ok := bedFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Bed) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := bedFieldToPtrFunc[col]; ok {
...
func (p *Bed) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Bed, bool, error) {
	defer rows.Close()
	beds := make([]*Bed, 0)
...
var bedFieldToValueFunc = map[dialect.Field]func(*Bed) (any, bool){
	...
	tblbed.BedTypeID: func(p *Bed) (any, bool) {
		return p.BedTypeID, p.BedTypeID == 0
	},
...
func (p *Bed) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := bedFieldToValueFunc[arg]; has {
			value, isZero := f(p)
