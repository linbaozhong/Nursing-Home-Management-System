...
func (p *VisitPlan) fieldPtr(args ...dialect.Field) []func(*VisitPlan) any {
	if len(args) == 0 {
		args = tblvisitplan.ReadableFields
	}
	fs := make([]func(*VisitPlan) any, 0, len(args))
	for _, col := range args {
		if f, ok := visitplanFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *VisitPlan) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := visitplanFieldToPtrFunc[col]; ok {
...
func (p *VisitPlan) Slice(rows *sql.Rows, args ...dialect.Field) ([]*VisitPlan, bool, error) {
	defer rows.Close()
...
func (p *VisitPlan) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := visitplanFieldToValueFunc[arg]; has {
			value, isZero := f(p)
