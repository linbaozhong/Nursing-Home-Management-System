...
func (p *HealthData) fieldPtr(args ...dialect.Field) []func(*HealthData) any {
	if len(args) == 0 {
		args = tblhealthdata.ReadableFields
	}
	fs := make([]func(*HealthData) any, 0, len(args))
	for _, col := range args {
		if f, ok := healthdataFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *HealthData) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := healthdataFieldToPtrFunc[col]; ok {
...
func (p *HealthData) Slice(rows *sql.Rows, args ...dialect.Field) ([]*HealthData, bool, error) {
	defer rows.Close()
...
func (p *HealthData) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := healthdataFieldToValueFunc[arg]; has {
			value, isZero := f(p)
