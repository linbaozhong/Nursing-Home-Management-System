...
func (p *Building) fieldPtr(args ...dialect.Field) []func(*Building) any {
	if len(args) == 0 {
		args = tblbuilding.ReadableFields
	}
	fs := make([]func(*Building) any, 0, len(args))
	for _, col := range args {
		if f, ok := buildingFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Building) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := buildingFieldToPtrFunc[col]; ok {
...
func (p *Building) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Building, bool, error) {
	defer rows.Close()
...
func (p *Building) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := buildingFieldToValueFunc[arg]; has {
			value, isZero := f(p)
