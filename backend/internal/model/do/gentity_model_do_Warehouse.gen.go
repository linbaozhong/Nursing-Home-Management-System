...
var warehouseFieldToPtrFunc = map[string]func(*Warehouse) any{
	...
	tblwarehouse.State.Name:      func(p *Warehouse) any { return &p.State },
}

// fieldPtr 根据字段参数，返回对应的指针获取函数列表（与具体实例无关，可缓存复用）
func (p *Warehouse) fieldPtr(args ...dialect.Field) []func(*Warehouse) any {
	if len(args) == 0 {
		args = tblwarehouse.ReadableFields
	}
	fs := make([]func(*Warehouse) any, 0, len(args))
	for _, col := range args {
		if f, ok := warehouseFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Warehouse) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := warehouseFieldToPtrFunc[col]; ok {
...
func (p *Warehouse) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Warehouse, bool, error) {
	defer rows.Close()
...
var warehouseFieldToValueFunc = map[dialect.Field]func(*Warehouse) (any, bool){
	...
	tblwarehouse.State: func(p *Warehouse) (any, bool) {
...
func (p *Warehouse) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := warehouseFieldToValueFunc[arg]; has {
			value, isZero := f(p)
