...
func (p *NurseItem) fieldPtr(args ...dialect.Field) []func(*NurseItem) any {
	if len(args) == 0 {
		args = tblnurseitem.ReadableFields
	}
	fs := make([]func(*NurseItem) any, 0, len(args))
	for _, col := range args {
		if f, ok := nurseitemFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *NurseItem) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := nurseitemFieldToPtrFunc[col]; ok {
...
func (p *NurseItem) Slice(rows *sql.Rows, args ...dialect.Field) ([]*NurseItem, bool, error) {
	defer rows.Close()
...
func (p *NurseItem) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := nurseitemFieldToValueFunc[arg]; has {
			value, isZero := f(p)
