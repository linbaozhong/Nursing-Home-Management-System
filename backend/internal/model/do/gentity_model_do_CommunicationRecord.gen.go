...
func (p *CommunicationRecord) fieldPtr(args ...dialect.Field) []func(*CommunicationRecord) any {
	if len(args) == 0 {
		args = tblcommunicationrecord.ReadableFields
	}
	fs := make([]func(*CommunicationRecord) any, 0, len(args))
	for _, col := range args {
		if f, ok := communicationrecordFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *CommunicationRecord) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := communicationrecordFieldToPtrFunc[col]; ok {
...
func (p *CommunicationRecord) Slice(rows *sql.Rows, args ...dialect.Field) ([]*CommunicationRecord, bool, error) {
	defer rows.Close()
...
func (p *CommunicationRecord) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := communicationrecordFieldToValueFunc[arg]; has {
			value, isZero := f(p)
