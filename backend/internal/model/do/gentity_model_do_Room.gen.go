...
func (p *Room) fieldPtr(args ...dialect.Field) []func(*Room) any {
	if len(args) == 0 {
		args = tblroom.ReadableFields
	}
	fs := make([]func(*Room) any, 0, len(args))
	for _, col := range args {
		if f, ok := roomFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Room) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := roomFieldToPtrFunc[col]; ok {
...
func (p *Room) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Room, bool, error) {
	defer rows.Close()
	rooms := make([]*Room, 0)
...
func (p *Room) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := roomFieldToValueFunc[arg]; has {
			value, isZero := f(p)
