...
func (p *Medicine) fieldPtr(args ...dialect.Field) []func(*Medicine) any {
	if len(args) == 0 {
		args = tblmedicine.ReadableFields
	}
	fs := make([]func(*Medicine) any, 0, len(args))
	for _, col := range args {
		if f, ok := medicineFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Medicine) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := medicineFieldToPtrFunc[col]; ok {
...
func (p *Medicine) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Medicine, bool, error) {
	defer rows.Close()
...
func (p *Medicine) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := medicineFieldToValueFunc[arg]; has {
			value, isZero := f(p)
