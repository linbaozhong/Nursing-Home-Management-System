...
func (p *Consume) fieldPtr(args ...dialect.Field) []func(*Consume) any {
	if len(args) == 0 {
		args = tblconsume.ReadableFields
	}
	fs := make([]func(*Consume) any, 0, len(args))
	for _, col := range args {
		if f, ok := consumeFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *Consume) AssignPtrByColumns(cols ...string) []any {
	_vals := make([]any, 0, len(cols))
	for _, col := range cols {
		if f, ok := consumeFieldToPtrFunc[col]; ok {
...
func (p *Consume) Slice(rows *sql.Rows, args ...dialect.Field) ([]*Consume, bool, error) {
	defer rows.Close()
...
func (p *Consume) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := consumeFieldToValueFunc[arg]; has {
			value, isZero := f(p)
