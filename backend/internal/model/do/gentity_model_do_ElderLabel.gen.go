...
func (p *ElderLabel) fieldPtr(args ...dialect.Field) []func(*ElderLabel) any {
	if len(args) == 0 {
		args = tblelderlabel.ReadableFields
	}
	fs := make([]func(*ElderLabel) any, 0, len(args))
	for _, col := range args {
		if f, ok := elderlabelFieldToPtrFunc[col.Name]; ok {
			fs = append(fs, f)
		}
	}
	return fs
...
func (p *ElderLabel) AssignPtrByColumns(cols ...string) []any {
	...
	for _, col := range cols {
		if f, ok := elderlabelFieldToPtrFunc[col]; ok {
...
func (p *ElderLabel) Slice(rows *sql.Rows, args ...dialect.Field) ([]*ElderLabel, bool, error) {
	defer rows.Close()
...
func (p *ElderLabel) AssignValues(d dialect.Dialect, args ...dialect.Field) ([]string, []any) {
	...
	for _, arg := range args {
		if f, has := elderlabelFieldToValueFunc[arg]; has {
			value, isZero := f(p)
